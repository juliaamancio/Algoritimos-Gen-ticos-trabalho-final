...
def cria_candidato(valor_min_x1, valor_max_x1, valor_min, valor_max, n):

    candidato = []
    for i in range(n):
        if i == 0:
            candidato.append(cria_gene(valor_min_x1, valor_max_x1))
        else:
            candidato.append(cria_gene(valor_min, valor_max))
    return candidato

def cria_populacao(tamanho, valor_min_x1, valor_max_x1, valor_min, valor_max, n = 2):
    populacao = []
    for _ in range(tamanho):
        populacao.append(cria_candidato(valor_min_x1, valor_max_x1, valor_min, valor_max, n))
...
def funcoes_objetivo(candidato):
    ...
    f1 = candidato[0]
    variaveis = candidato[1:]
    somatoria = [(valor**2 - 10*np.cos(4*np.pi*valor)) for valor in variaveis]

    g = 1 + 10*(len(candidato) - 1) + sum(somatoria)
    
    f2 = g * (1 - np.sqrt( candidato[0] / g ))
